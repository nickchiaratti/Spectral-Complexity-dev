...
class TanagerNativeViewer:
    def __init__(self, h5_path):
        ...
        if grid_path not in self.h5:
            raise KeyError(f"Standard TANAGER grid path '{grid_path}' not found in the provided HDF5 file.")
        ...
        if "surface_reflectance" not in self.grp:
            raise KeyError("'surface_reflectance' dataset missing.")
            
        self.vis_dset = self.grp["ortho_visual"]
        self.sr_dset = self.grp["surface_reflectance"]
        ...
        if "acquisition_time" not in self.sr_dset.attrs:
            raise AttributeError("CRITICAL: 'acquisition_time' attribute missing from surface_reflectance.")
        ...
        top = gt[3]
        pixel_width = gt[1]
        pixel_height = gt[5]  # Expected to be negative for North-up imagery
    ...
    def setup_ui(self):
        ...
        # Format axes for rigorous geospatial inspection
        self.ax.set_xlabel("UTM Easting (Meters)", fontweight='bold')
        self.ax.set_ylabel("UTM Northing (Meters)", fontweight='bold')
        self.ax.xaxis.set_major_formatter(plt.FormatStrFormatter('%1.0f'))
...
<html>
<head>
    <title>Tanager Frames Map</title>
    <meta charset="utf-8" />
...
if __name__ == "__main__":
    ...
    print("Please select the Tanager Native Stack HDF5 file...")
    file_path = filedialog.askopenfilename(
        title="Select Tanager Native Stack",
        filetypes=[("HDF5 files", "*.h5")]
    )
    
    if file_path:
        viewer = TanagerNativeViewer(file_path)
    else:
