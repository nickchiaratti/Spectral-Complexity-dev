...
def parse_enmap_scene(json_path):
    ...
    # Locate METADATA.XML for exact spatial coverage and band metadata fallback
...
def process_mgrs_stack(target_location):
    ...
    if not os.path.exists(location_dir):
        ...
        if os.path.exists(alt_dir):
            ...
            print(f"Warning: EnMAP location directory not found: {location_dir} or {alt_dir} for {target_location}. Skipping.")
            return None
    ...
    if not valid_json_files:
        print(f"Warning: No valid EnMAP JSON files with adjacent TIFFs found in {location_dir} for {target_location}. Skipping.")
        return None
    ...
    if not raw_scenes:
        print(f"Warning: No complete EnMAP scenes found for {target_location}. Skipping.")
        return None
    ...
    with h5py.File(output_file, 'w') as out_h5:
        ...
        ds_rad = grp_enmap.create_dataset("surface_reflectance", shape=(n_times, n_bands, height, width), dtype='int16', compression="gzip", compression_opts=5, shuffle=True, fillvalue=rad_nodata)
        mask_ds_dict = {}
        for mk in mask_keys:
            mask_ds_dict[mk] = grp_enmap.create_dataset(mk, shape=(n_times, height, width), dtype='uint8', compression="gzip", shuffle=True, fillvalue=255)
        ds_vis = grp_enmap.create_dataset("ortho_visual", shape=(n_times, 4, height, width), dtype='uint8', compression="gzip",shuffle=True, fillvalue=0)
        ...
        for t_idx, group in enumerate(grouped_scenes):
            ...
            canvas_rad = np.full((n_bands, height, width), rad_nodata, dtype='int16')
            ...
            for scene in group:
                ...
                try:
                    with rasterio.open(scene['reflectance_tif']) as src:
                        incoming_rad = np.full((n_bands, height, width), rad_nodata, dtype='int16')
                        reproject(
                ...
                except (rasterio.errors.RasterioIOError, rasterio.errors.WarpOperationError):
        ...
        mask_ds = grp_enmap.create_dataset('common_mask', shape=(n_times, height, width), dtype=bool, shuffle=True, compression="gzip", fillvalue=True)
        ...
        for t_idx in tqdm(range(n_times), desc="  Writing visual channels"):
            rgba_img = sc.generate_rgba_from_hsi(frame_data=ds_rad[t_idx, :, :, :], wavelengths=base_wv, nodata=rad_nodata)
        ...
        ds_rad.attrs["scale_to_float"] = 0.0001
