...
class ComplexityResultViewer:
    def __init__(self, h5_path):
        ...
        self.grid_dset = self.data_grp.get('spectral_complexity_grid')
        self.slide_dset = self.data_grp.get('spectral_complexity_sliding')
        self.vol_dset = self.data_grp.get('endmember_volumes')
    ...
    def update_display(self):
        ...
        r = nsc.percentile_normalize_array(frame_sr[RGB_BANDS[0]])
        g = nsc.percentile_normalize_array(frame_sr[RGB_BANDS[1]])
        b = nsc.percentile_normalize_array(frame_sr[RGB_BANDS[2]])
        ...
        if self.vol_dset is not None:
            ...
            self.ax_vol_curve.set_title("ROI Volume Curve"); self.ax_vol_curve.grid(True, alpha=0.3)
    ...
    def _on_save_images(self, event):
        idx = self.current_frame; ts = self.acq_times[idx]
        dt = datetime.fromtimestamp(ts, tz=timezone.utc); time_str = dt.strftime('%Y-%m-%dT%H%M%S')
