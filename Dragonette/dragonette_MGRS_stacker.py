...
def process_dragonette_mgrs_stack(target_location):
    ...
    if not valid_json_files:
        print(f"Warning: No valid Wyvern STAC JSON files with adjacent TIFFs found in {location_dir} for {target_location}. Skipping.")
        return None
    ...
    with h5py.File(output_file, 'w') as out_h5:
        ...
        src_rad_nodata = raw_scenes[0]['nodata']
        rad_nodata = -9999
        ds_rad = grp_wyvern.create_dataset("surface_reflectance", shape=(n_times, n_bands, height, width), dtype='int16', compression="gzip", compression_opts=5, shuffle=True, fillvalue=rad_nodata)
        ds_dmask = grp_wyvern.create_dataset("data_mask", shape=(n_times, 4, height, width), dtype='uint8', compression="gzip", compression_opts=5, shuffle=True, fillvalue=255)
        ds_qmask = grp_wyvern.create_dataset("pixel_quality_mask", shape=(n_times, n_bands, height, width), dtype='uint8', compression="gzip", shuffle=True, compression_opts=5, fillvalue=255)
        ds_vis = grp_wyvern.create_dataset("ortho_visual", shape=(n_times, 4, height, width), dtype='uint8', compression="gzip", compression_opts=5, shuffle=True, fillvalue=0)
        ds_common_mask = grp_wyvern.create_dataset("common_mask", shape=(n_times, height, width), dtype=bool, compression="gzip", compression_opts=5, shuffle=True, fillvalue=False)
        ...
        for t_idx, scene in enumerate(raw_scenes):
            ...
            with rasterio.open(scene['radiance_tif']) as src:
                ...
                reproject(
                    ...
                    src_nodata=src_rad_nodata, dst_nodata=rad_nodata
                )
