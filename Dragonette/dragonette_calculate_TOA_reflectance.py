...
def apply_apparent_reflectance(h5_path):
    ...
    with h5py.File(h5_path, 'r+') as h5:
        ...
        # Clear out any legacy reflectance datasets to maintain namespace hygiene
        for legacy_key in ['surface_reflectance', 'surface_reflectance_apparent', 'toa_reflectance']:
            if legacy_key in h5['HDFEOS/GRIDS/WYVERN/Data Fields']:
                del h5['HDFEOS/GRIDS/WYVERN/Data Fields'][legacy_key]
        
        toa_ds = h5['HDFEOS/GRIDS/WYVERN/Data Fields'].create_dataset(
            "toa_reflectance", 
            shape=rad_ds.shape, 
            dtype='float32', 
            compression="gzip",
        ...
        for t in range(num_times):
            ...
            for b in range(num_bands):
                ...
                # Core Equation: rho = (PI * L * d^2) / (ESUN * cos(theta))
                # Executed exclusively on valid pixels. Calculates pure Wyvern Top-of-Atmosphere Reflectance.
                reflectance_band[mask] = (np.pi * radiance_band[mask] * (d**2)) / denominator
                
                toa_ds[t, b, :, :] = reflectance_band
        ...
        toa_ds.attrs['unit'] = "unitless"
        toa_ds.attrs['description'] = "Top-of-Atmosphere (TOA) Reflectance (Wyvern standard radiometric formulation)"

        # 4. Ortho Visual Generation
        print("Extracting 'ortho_visual' specific bands from TOA reflectance...")
        ...
        for t in range(num_times):
            ...
            vis_ds[t, 0, :, :] = toa_ds[t, r_idx, :, :]
            vis_ds[t, 1, :, :] = toa_ds[t, g_idx, :, :]
            vis_ds[t, 2, :, :] = toa_ds[t, b_idx, :, :]
        ...
        vis_ds.attrs['description'] = "True Color RGB composite derived from TOA Reflectance"
