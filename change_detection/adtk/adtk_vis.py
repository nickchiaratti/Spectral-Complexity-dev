...
from adtk_main import LOCATION, H5_PATH, TARGET_METRIC
import glob

def get_inference_h5(location):
    search_pattern = f"C:/satelliteImagery/HLST30/CCD/{location}_CCD_adtk_Change_Detection_{TARGET_METRIC}.h5"
...
def plot_pixel_sits(pixel_y, pixel_x, source_h5_path, inference_results_h5, ax=None, current_date=None):
    ...
    # Mark anomalies flagged by ADTK
    anom_mask = anomalies == 1
    if np.any(anom_mask):
        anom_dates = dates_arr[anom_mask]
        anom_vals = z_score[anom_mask]
        ax.plot(anom_dates, anom_vals, 'rx', markersize=10, mew=2, label='Anomaly Flagged')
        # Draw vertical lines at anomalies
...
if __name__ == "__main__":
    ...
    if inference_h5 and os.path.exists(inference_h5):
        ...
        print("Run adtk_main.py first to create output h5")
