...
2. **Inception Block (Multi-Scale Feature Extraction)**: 
   - Three parallel `Conv1d` branches with different kernel sizes (`3`, `5`, and `7`), each taking `in_channels` and outputting `16` channels with `padding='same'`.
   - The outputs of the three branches are concatenated along the channel dimension to form a `48`-channel tensor.
...
4. **Secondary Extractor**:
   - A single `Conv1d` layer (in: 48, out: 64, kernel: 3, padding='same') refines the extracted multi-scale features.
