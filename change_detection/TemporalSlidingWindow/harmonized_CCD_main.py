...
TARGET_METRIC = 'sliding_volume_z_score'#'pixel_temporal_z_score'  'sliding_volume_z_score' 'temporal_z_score' 'ndvi_map'
RMSE_MULTIPLIER = 2.5
...
def build_harmonic_matrix(t, temporal_periods=TEMPORAL_PERIODS, 
    ...
    Note: enable_const is now ignored, intercept is handled natively by LASSO fit_intercept=True.
    """
    cols = []
    # Intercept is handled by sklearn's fit_intercept=True
    ...
    for y_local in range(chunk_height):
        ...
        for x in range(width):
            ...
            for i in range(min_samples, len(valid_indices)):
                ...
                # Fit model on window using LASSO with BIC penalty selection
                ...
                # Predict target step
                X_target = X_full[target_idx, :].reshape(1, -1)
                y_pred = lasso_model.predict(X_target)[0]
