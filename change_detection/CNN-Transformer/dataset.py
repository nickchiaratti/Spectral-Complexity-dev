...
class SITSDataset(Dataset):
    def __init__(self, h5_path, mode='calibration', train_end_date='2024-01-01', 
                 consecutive_anomalies=3, time_window_years=3.0, enable_elastic_window=True, 
                 max_elastic_window_years=5.0, min_samples=25):
        """
        ...
        self.temporal_periods = [1.0, 0.5, 0.33]
    ...
    def __getitem__(self, idx):
        ...
        dt_years_norm_sq = dt_years_norm ** 2
        constant_term = torch.ones_like(dt_years_norm)
        ...
        feature_list = [
            ...
            pixel_tod_sin,
            pixel_tod_cos,
            constant_term.to(torch.float32),
            dt_years_norm.to(torch.float32),
            dt_years_norm_sq.to(torch.float32)
        ] + dt_features + [pixel_z]
        ...
        if seq_len > 0:
            ...
            # Sequence mask (padding mask, binary 1.0 for valid elements)
        ...
        dt_target_norm_sq = dt_target_norm ** 2
        target_constant = torch.ones_like(dt_target_norm)
        ...
        target_feature_list = [
            ...
            target_tod_sin,
            target_tod_cos,
            target_constant.to(torch.float32),
            dt_target_norm.to(torch.float32),
            dt_target_norm_sq.to(torch.float32)
        ] + target_dt_features
