...
from pyxccd_main import LOCATION, H5_PATH, TARGET_METRIC
import glob

def get_inference_h5(location):
    search_pattern = f"C:/satelliteImagery/HLST30/CCD/{location}_CCD_pyxccd_Change_Detection_{TARGET_METRIC}.h5"
...
def plot_pixel_sits(pixel_y, pixel_x, source_h5_path, inference_results_h5, ax=None, current_date=None):
    ...
    if np.any(pred_mask):
        ...
        ax.plot(pred_dates, preds, 'b--', label='Harmonic Prediction (pyxccd)')
...
def plot_spatial_anomaly_overlay(source_h5_path, inference_results_h5):
    ...
    if not np.all(np.isnan(anomaly_map)):
        ...
        cmap = copy.copy(mpl.colormaps['spring']) # using a bright colormap to stand out
...
if __name__ == "__main__":
    ...
    if inference_h5 and os.path.exists(inference_h5):
        ...
        print("Run pyxccd_main.py first to create output h5")
